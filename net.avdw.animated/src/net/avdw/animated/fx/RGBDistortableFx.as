package net.avdw.fx
{
	...
	public class RGBDistortableFx extends Sprite
	{
		...
		private function tween(bmp:Bitmap):void
		{
			...
			if (distorting)
			{
				if (config.hasOwnProperty("x"))
					vars.x(randomInteger(-config.x / 2, config.x / 2));
				
				if (config.hasOwnProperty("y"))
					vars.y(randomInteger(-config.y / 2, config.y / 2));
				
				if (config.hasOwnProperty("scaleX"))
					vars.scaleX(randomNumber(1 - config.scaleX / 2, 1 + config.scaleX / 2));
				
				if (config.hasOwnProperty("scaleY"))
					vars.scaleY(randomNumber(1 - config.scaleY / 2, 1 + config.scaleY / 2));
				
				if (config.hasOwnProperty("alpha"))
					vars.autoAlpha(randomNumber(1 - config.alpha, 1));
			...
			TweenLite.to(bmp, randomNumber(.1, config.hasOwnProperty("speed") ? 1.1 - config.speed : .2), vars);
		}
	}
}